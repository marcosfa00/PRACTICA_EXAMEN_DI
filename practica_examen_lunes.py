...
class VentanaPrincipal(QMainWindow):
    def __init__(self):
        ...
        btnAddList.clicked.connect(lambda: self.addSong(lista))
        btnUpList = QPushButton("Up")
        btnUpList.clicked.connect(lambda : self.upButton(lista))
        btnDownList = QPushButton("Down")
        btnDownList.clicked.connect(lambda : self.downButton(lista))
        ...
        btndelete.clicked.connect(lambda : self.deleteItem(lista))
        ...
        sliderRitmo = QSlider()
        sliderRitmo.setMinimum(0)
        sliderRitmo.setMaximum(8)
        sliderRitmo.valueChanged.connect(self.printValue)
    ...
    # Aquí vamos a definir todas las funciones
    def addSong(self,list):
        song = input("Que cancion queires Añadir? ")
        list.addItem(song)
    def upButton(self,list):
        currentRow = list.currentRow()
        if currentRow > 0:
            list.setCurrentRow(currentRow -1)

    def downButton(self,list):
        # obtenemos la posicion actual de la lista
        current_row = list.currentRow()
        # comprobamos que no es el ultimo elemento de la lista
        if current_row < list.count() - 1:
            list.setCurrentRow(current_row + 1)

    def deleteItem(self,list):
        # obytenemos la posicion actual en la lista
        currentRow = list.currentRow()
        # verificamos si hay un elemento seleccionado
        if currentRow >= 0:
            list.takeItem(currentRow)  # se elimina el elemento de la lista
        else:
